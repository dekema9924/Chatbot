    ...
    return (
        <>
            ...
            <div className="flex flex-col h-full">
                {/* User response */}
                <div className="mt-10 my-4 ml-auto relative flex-grow overflow-y-auto">
                    {myMessage.length > 0 &&
                ...
                {/* Gemini (Bot) Response */}
                <div className="flex flex-col gap-2 relative items-center flex-grow overflow-y-auto">
                    <p onClick={ClearChat} className=' text-xs bg-black rounded-md p-2 cursor-pointer uppercase'>Clear Chat</p>
                    {loading ? (
                        ...
                        botResponse.map((bot, index) => (
                            <div key={index} className="flex items-center bg-black rounded-lg w-9/12 p-4">
            ...
            {/* Chat Input */}
            <form onSubmit={HandleSubmit} className="fixed w-full flex justify-center bottom-4 left-0 z-10">
                <input
                    onChange={(e) => setChatText(e.target.value)}
                    className="w-11/12 rounded-2xl bg-[#121212] h-20 outline-none pl-7"
                    type="text"
                    value={chatText}
